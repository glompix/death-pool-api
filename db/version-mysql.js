...
module.exports.migrate = function() {
	var c = mysql.createConnection(_.extend(config.connection, {
...
function ifNeedToMigrate(c, callback) {
	bootstrap(c, function(err) {
		if (err) throw err;
		c.query('select * from version limit 1', function(err, rows) {
			if (err) throw err;
			var version;
			if (rows.length === 0) {
...
function doMigrate(c, currentVersion) {
	...
	try {
		...
		if (scripts.latest > currentVersion) {
			var ups = _.filter(scripts.ups, function(f) {
				return getNumber(f) > currentVersion;
			});
			...
			var downs = _.filter(scripts.downs, function(f) {
				return getNumber(f) < currentVersion;
			}).reverse();
...
function readScriptFolder() {
	...
	var scripts = {
		ups: _.filter(files, function(f) {
			return f.match(/up/i);
		}).sort(),
		downs: _.filter(files, function(f) {
			return f.match(/down/i);
		}).sort()
	};
...
function runScripts(c, scripts, callback) {
	...
	replaceTokens(script);
	c.query(script, function(err) {
		if (err) throw err;
...
function replaceTokens(text) {
	text = text.replace('{{dbname}}', config.connection.database);
	return text;
}
